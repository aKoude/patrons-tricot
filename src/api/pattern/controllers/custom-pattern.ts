...
export default factories.createCoreController('api::pattern.pattern', ({ strapi }) => ({
  ...
  async findUnlocked(ctx) {
    ...
    try {
      ...
      const userAccesses = allAccesses.filter((access: any) => access.user?.id === user.id);
      
      const patterns = userAccesses.map((access: any) => access.pattern);
  ...
  async unlock(ctx) {
    ...
    try {
      // Récupérer le patron
      const pattern: any = await strapi.db.query('api::pattern.pattern').findOne({
        where: { documentId: documentId },
      });
      ...
      const existingAccess = allAccesses.find((access: any) => 
        access.user?.id === user.id && 
        access.pattern?.documentId === pattern.documentId
      );
      ...
      const userWithCredits: any = await strapi.db.query('plugin::users-permissions.user').findOne({
        where: { id: user.id },
      });
  ...
  async start(ctx) {
    ...
    try {
      ...
      const access = allAccesses.find((access: any) => 
        access.user?.id === user.id && 
        access.pattern?.documentId === documentId
      );
      ...
      const existingProgress = allProgresses.find((progress: any) => 
        progress.user?.id === user.id && 
        progress.pattern?.documentId === documentId
      );
      ...
      if (pattern.hasSizes) {
        const sizeExists = pattern.patternSizes.some((s: any) => s.id === sizeId);
        if (!sizeExists) {
          return ctx.badRequest({
            success: false,
            error: 'Invalid size',
      ...
      let sections: any[];
      if (pattern.hasSizes) {
        const selectedSize = pattern.patternSizes.find((s: any) => s.id === sizeId);
      ...
      const sectionsProgress = sections.map((section: any) => {
        const totalStitches = section.steps.reduce((sum: number, step: any) => {
    ...
    try {
      ...
      return ctx.send({
        ...
        data: accesses.map((access: any) => ({
          ...
          patternId: access.pattern?.id,
          patternDocumentId: access.pattern?.documentId,
          patternTitle: access.pattern?.title,
  ...
  async debugProgresses(ctx) {
    try {
      ...
      return ctx.send({
        ...
        data: progresses.map((progress: any) => ({
          id: progress.id,
          documentId: progress.documentId,
          userId: progress.user?.id,
          userEmail: progress.user?.email,
          patternId: progress.pattern?.id,
          patternDocumentId: progress.pattern?.documentId,
          patternTitle: progress.pattern?.title,
